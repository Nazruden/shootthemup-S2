...
const int BASIC_PROJECTILE_WIDTH = 20;
...
class Projectile : public MovableElement
{
    ...
    public:
    // Constructors
        Projectile(GamePlay* gamePlay, int x, int y, int w, int h, int dx, int dy, std::string name, int damages, Spaceship* shooter);
        Projectile(Projectile* autre);
        Projectile(int x, int y, int w, int h, int dx, int dy, std::string name, int damages, Spaceship* shooter);
        ~Projectile();
    // Accessors
        int getDamages();
